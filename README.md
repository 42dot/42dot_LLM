...
## 한계점
다른 LLM과 마찬가지로 챗베이커 (ChatBaker) 도 여러 한계를 가지고 있습니다. 챗베이커 (ChatBaker) 모델을 연구에 활용하실 때 이러한 한계점들을 감안하시기 바랍니다.
...
- 생성형 언어 모델인 챗베이커 (ChatBaker)는 랜덤 샘플링 방식을 따르고 있습니다. 이로 인해, 동일한 입력에 대해 매번 다른 응답을 생성 할 수 있습니다. 또한, 사용자가 입력한 질문/요청인 프롬프트에 민감합니다. 예를 들어, 주어진 질문에 정확한 답변을 생성했더라도, 동일한 내용에 표현 방식만 다른 질문/요청에 전혀 다른 응답을 생성 할 수 있습니다.
